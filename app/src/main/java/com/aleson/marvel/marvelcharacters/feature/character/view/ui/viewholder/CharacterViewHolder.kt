...
class CharacterViewHolder(var context: Context?, view: View, var onFavorite: (Character) -> Unit) :
    ...
    private val item: ConstraintLayout = view.findViewById(R.id.character_details_constraintlayout_item)

    override fun bind(data: Character, listener: BaseRecyclerListener<Character>?, position: Int) {
        name.text = data.name
        loadImageFromUrl(context as Context, data.thumbnail, image)
        item.setOnClickListener {
            listener?.onClickListener(data, it, position)
        }
